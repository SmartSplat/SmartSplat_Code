...
class SimpleTrainer2d:
    ...
    def __init__(
        ...
        if compression_ratio > 0:
            ...
            self.log_dir = Path(f"./logs/{args.data_name}/{model_name}/Iter{args.iterations}_P{num_points}_CR{self.compression_ratio}/{self.image_name}")
            if self.log_dir.exists() and self.log_dir.is_dir():
                shutil.rmtree(self.log_dir)
            self.training_render_dir = Path(f"./logs/{args.data_name}/{model_name}/Iter{args.iterations}_P{num_points}_CR{self.compression_ratio}/{self.image_name}/renders")
        ...
        # Ours
            ...
            print("Coping source code .... ")
            src_path = "./gaussian_models/gaussianimage_rs_sample.py"
            dst_path = f"{self.log_dir}/gaussianimage_rs_sample.py"
...
def main(argv):
    ...
    # logwriter = LogWriter(Path(f"./logs/{args.data_name}/{args.model_name}/{args.model_name}_{args.iterations}_{args.num_points}"))
    ...
    # elif args.data_name == "DIV2K_valid_LRX2":
        ...
        image_length, start = 1, 800
    elif args.data_name == "tokyo":
        image_length, start = 1, 0
    elif args.data_name == "DIV8K":
        image_length, start = 1, 0
    elif args.data_name == "DIV16K":
        image_length, start = 1, 0
    elif args.data_name == "GLH16K":
        image_length, start = 1, 0
    ...
    for i in range(start, start+image_length):
        ...
        # elif args.data_name == "DIV2K_valid_LRX2":
            ...
            # image_path = Path(args.dataset) /  "megapixel_tokyo.jpg"
            # image_path = Path(args.dataset) /  "01.jpg"
            image_path = Path(args.dataset) /  "02.jpg"
        elif args.data_name == "DIV8K":
            # image_path = Path(args.dataset) / f'{i+1:04}.png'
            # image_path = Path(args.dataset) / f'0353.png'
            image_path = Path(args.dataset) / f'0952.png'
        elif args.data_name == "DIV16K":
            # image_path = Path(args.dataset) / f'{i+1:04}.png'
            # image_path = Path(args.dataset) / f'0353.png'
            image_path = Path(args.dataset) / f'0823_16320x10272.png'
        elif args.data_name == "GLH16K":
            image_path = Path(args.dataset) / 't-2691.png'
        
        trainer = SimpleTrainer2d(image_path=image_path, num_points=args.num_points, compression_ratio=args.compression_ratio,
            iterations=args.iterations, model_name=args.model_name, args=args, model_path=args.model_path)
        
        trainer.train()

    # TODO Many Image Eval
