...
def main(argv):
    ...
    for i, img_path in enumerate(image_files):
        ...
        test_path = Path(log_path) / "test.txt"
        ...
        if test_path.exists():
            with open(test_path, 'r') as f:
                for line in f:
                    ...
                    if len(parts) >= 5:
                        # Extract the image number (e.g., "0001" from "0001: 5376x5760")
                        current_image = parts[0].split(':')[0]
                        
                        if current_image == image_name:
                            print(line)
                            # Extract PSNR value
                            psnr = float(parts[1].split(':')[1])
                            # Extract MS-SSIM value
                            ms_ssim = float(parts[2].split(':')[1])
                            # Extract Training time
                            training_time = float(parts[3].split(':')[1].replace('s', ''))
                            # Extract Eval time
                            eval_time = float(parts[4].split(':')[1].replace('s', ''))
                            # Extract FPS
                            fps = float(parts[5].split(':')[1])
                            ...
                            eval_fpses.append(fps)

                            image_pass = True
                            break

        if image_pass:
            print(f"{image_name} has been evaled, pass.........")
            continue
